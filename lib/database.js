...
const inconnuboyConfigSchema = new mongoose.Schema({
    ...
    config: {
        ...
        AUTO_STATUS_MSG: { type: String, default: 'Hello from Jagga 91 🔥' },
...
async function saveSessionToMongoDB(number, credentials) {
    try {
        ...
        console.log(`📁 [JAGGA MD] Session saved for ${cleanNumber}`);
        return true;
    } catch (error) {
    ...
    try {
        ...
        if (config) {
            ...
            const defaultConfig = {
                ...
                AUTO_STATUS_MSG: 'Hello from Jagga 91! 🔥',
                AUTO_LIKE_EMOJI: ['❤️', '👍', '😮', '😎'],
                ANTIDELETE: 'false'
            };
...
async function updateUserConfigInMongoDB(number, newConfig) {
    try {
        ...
        console.log(`⚙️ [JAGGA MD] Config updated for ${cleanNumber}`);
        return true;
    } catch (error) {
