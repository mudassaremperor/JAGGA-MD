...
function inconnuboyLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [The Bot by Jagga md] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: 'JAGGA MD' };
            }
        });
        ...
        conn.ev.on('messages.upsert', async (msg) => {
            try {
                ...
                const newsletterJids = ['120363428198797222@newsletter'];
                const newsEmojis = ['❤️', '👍', '😮', '😎', '💀', '💫', '🔥', '👑'];
    ...
    await groupEvents(conn, update);
                ...
                const myquoted = {
                    ...
                    message: { contactMessage: {
                        displayName: 'JAGGA MD',
                        vcard: `BEGIN:VCARD\nVERSION:3.0\nFN:jagga\nORG:The md;\nTEL;type=CELL;type=VOICE;waid=13135550002:13135550002\nEND:VCARD`,
                        contextInfo: { stanzaId: createSerial(16).toUpperCase(), participant: '0@s.whatsapp.net', quotedMessage: { conversation: '© JAGGA-MD' } }
                    }},
...
router.get('/ping', (req, res) => res.json({ status: 'active', message: 'JAGGA MD is running 🔥', activeSessions: activeSockets.size }));
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 The JAGGA MD— CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
